...
- Color: `#666666` (TextOnLightTertiary)
- Padding top: 2dp
- Padding left: 26dp (aligns with plan name text, after icon width + margin)
...
- Font: 16sp / Medium
- Color: #000000
- Max width: Calc(full card width - 60dp) to avoid overlap with × icon
...
- **× icon tap** → Shows confirmation, removes exercise from queue
...
- Minimum height: 80dp (comfortable tap target)
- Press feedback: Scale 0.98, duration 100ms
- **× icon tap** → Removes exercise and deletes all logged sets (after confirmation)
...
- Size: 20dp
- Position: Right of exercise name (not overlapping with ×)
...
- Minimum height: 80dp (comfortable tap target)
- Press feedback: Scale 0.98, duration 100ms
- **× icon tap** → Shows confirmation with completion + deletion warning
...
- Minimum height: 80dp (comfortable tap target)
- Press feedback: Scale 0.98, duration 100ms
- **× icon tap** → Shows confirmation, removes exercise from workout
...
**Context**: User tapped the × icon on an exercise card in Active Workout screen. Confirmation dialog appears before deletion.
...
- If exercise was In Progress → No automatic state change (Issue #5 - single In Progress rule doesn't auto-promote)
- If no exercises remain → Show empty state
